...
def load_image():
    ...
    has_stim.setPos(newPos=[positionH, 0.1])
    sas_stim.setPos(newPos=[positionS, 0.1])
...
def load_dot():
    ...
    probe = visual.TextStim(win, text=u"q",
                           height=0.07,
                           pos=[positionO, -0.1],
                           color='grey')
    lure = visual.TextStim(win, text=u"p",
                           height=0.07,
                           pos=[positionX, -0.1],
                           color='grey')
...
def run_trials(n_trials):
    ...
    while continueRoutine:
        for trial in range(n_trials + 1):
            ...
            routineTimer.reset()
            routineTimer.add(stimuliDuration)
            stimStart = True
            while stimStart == True:
                theseKeys = []
                has_stim.draw()
                sas_stim.draw()
                win.flip()
